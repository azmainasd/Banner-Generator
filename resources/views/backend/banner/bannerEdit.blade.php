    ...
    <div class="col-md-12 grid-margin stretch-card">
      <div class="card">
        <div class="card-body">
          ...
          <form method="POST" action={{ route('banner_update', $category->id) }} enctype="multipart/form-data" class="forms-sample">
            @method("PUT")
            @csrf
                        ...
                        <tr id="row_to_copy" class="tr0">
                            <td>
                                <div class="form-group">
                                  ...
                                  <input type="file" name="images[]" class="d-none" id="images" value="{{ $product->image_path }}">
                                  <div class="input-group w-380px col-xs-12">
                                    <input type="text" class="form-control file-upload-info" value="{{ $product->image_name }}" disabled placeholder="Upload Image">
                                    <span class="input-group-append" onClick="fileUpload($(this))">
                                      <button class="file-upload-browse btn btn-primary" type="button">Upload</button>
                                    </span>
                                  </div>
                                </div>
                                <input type="text" name="links[]" class="form-control w-380px pb-1 mb-2" id="links" placeholder="URL" value="{{ $product->url }}">
                                {{-- @error('images') --}}
                            ...
                            <td><img class="img-show" src="{{ asset($product->image_path) }}" style="display:block; border-radius:0; height:150px; width:150px;" height="100%" width="100%" /></td>

                            <td>
                                <input type="text" class="form-control" name="prices[]" id="prices" value="{{ $product->price }}">
                            ...
                            <td>
                                <input type="text" class="form-control" name="product_titles[]" id="product_titles" value="{{ $product->title }}">
                                <span class="invalid-feedback product_titles d-block" role="alert">
                                  <strong></strong>
                                </span>
                            </td>

                            <td class="pl-2 pr-3" style="width: 10%">
                                <input type="hidden" name="product_ids[]" value="{{ $product->id }}">
                                <button onclick="rowAdd($(this))" class="btn_add_more"><i class="fa fa-plus" style="color:white"></i></a>
                                <button onclick="rowRemove($(this))" style="background-color:red;border:none;height:30px;width:30px;display:block;"><i class="fa fa-minus" style="color:white"></i></button>
                            </td>
                        </tr>
                      @endforeach
                    </tbody>
            ...
            <div class="button-group mt-3">
              ...
              <a href="{{ route("banner-list") }}" class="btn btn-light">Cancel</a>
            </div>
          </form>
        </div>
      </div>
...
<script>
  ...
  function Validator(e){
   ...
   for(let idx = 0; idx <= count; idx++ ){
      console.log("img val", $(`.tr${idx} #images`).val())

      $(`.tr${idx} .file-upload-info`).val() || $(`.tr${idx} #links`).val() ? 
